...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
	...
	case SC_SEM_CREATE:{
		...
		char nomDebug[256];
		int32_t initVal = g_machine->ReadIntRegister(5), objNumber;
		GetStringParam(g_machine->ReadIntRegister(4), nomDebug, 256);
		
		if(initVal < 0){
		
			DEBUG('e', (char*)"SEM: Error on create semaphore.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidParameter);
		}
		else{
		
			DEBUG('e', (char*)"SEM: Create semaphore %s.\n", nomDebug);
			Semaphore *sem = new Semaphore(nomDebug, initVal);
			
			//Ajout du sémaphore dans la liste des objets crées par le système
			objNumber = g_object_ids->AddObject(sem);
			g_machine->WriteIntRegister(2, objNumber);
			g_syscall_error->SetMsg((char*)"",NoError);
		}
	...
	case SC_SEM_DESTROY:{
		...
		int32_t numSem = g_machine->ReadIntRegister(4);
		Semaphore *sem = (Semaphore *)g_object_ids->SearchObject(numSem);
		
		if(sem && (sem->typeId == SEMAPHORE_TYPE_ID)){
		
			DEBUG('e', (char*)"SEM: DESTROY semaphore %s.\n", sem->getName());
			
			//Suppression de l'objet ainsi que ses références
			g_object_ids->RemoveObject(numSem);
			delete sem;
			...
			DEBUG('e', (char*)"SEM: Error on DESTROY semaphore.\n", sem->getName());
			g_machine->WriteIntRegister(2,-1);
	...
	case SC_P:{
		...
		int32_t idObj = g_machine->ReadIntRegister(4);
		Semaphore *sem = (Semaphore *)g_object_ids->SearchObject(idObj);
		
		if(sem && (sem->typeId == SEMAPHORE_TYPE_ID)){
		
			DEBUG('e', (char*)"SEM: P on semaphore %s\n", sem->getName());
			sem->P();
			g_machine->WriteIntRegister(2, 0);
			g_syscall_error->SetMsg((char*)"",NoError);
		}
	...
	case SC_V:{
		...
		if(sem && (sem->typeId == SEMAPHORE_TYPE_ID)){
			...
			DEBUG('e', (char*)"SEM: V on semaphore %s.\n", sem->getName());
			sem->V();
			g_machine->WriteIntRegister(2,0);
			g_syscall_error->SetMsg((char*)"",NoError);
		}
	...
	case SC_LOCK_CREATE:{
		...
		DEBUG('e', (char*)"LOCK: Create lock %s.\n", nomDebug);
		Lock *lock = new Lock(nomDebug);
		
		//Ajout du verrou dans la liste des objets crées par le système
		objNumber = g_object_ids->AddObject(lock);
		g_machine->WriteIntRegister(2, objNumber);
		g_syscall_error->SetMsg((char*)"",NoError);	
	...
	case SC_LOCK_DESTROY:{
		...
		int32_t numLock = g_machine->ReadIntRegister(4);
		Lock *lock = (Lock *)g_object_ids->SearchObject(numLock);
		
		if(lock && (lock->typeId == LOCK_TYPE_ID)){
		
			DEBUG('e', (char*)"LOCK: DESTROY lock %s.\n", lock->getName());
			
			//Suppression de l'objet ainsi que ses références
			g_object_ids->RemoveObject(numLock);
			delete lock;
			...
			DEBUG('e', (char*)"LOCK: Error on DESTROY.\n");
			g_machine->WriteIntRegister(2,-1);
	...
	case SC_LOCK_ACQUIRE:{
		...
		if(lock && (lock->typeId == LOCK_TYPE_ID)){
			...
			DEBUG('e', (char*)"LOCK: ACQUIRE on lock %s.\n", lock->getName());
	...
	case SC_LOCK_RELEASE:{
		...
		if(lock && (lock->typeId == LOCK_TYPE_ID)){
			...
			DEBUG('e', (char*)"LOCK: RELEASE on lock %s.\n", lock->getName());
			...
			DEBUG('e', (char*)"LOCK: Error on RELEASE.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidLockId);
		}
	...
	case SC_COND_CREATE:{
		...
		DEBUG('e', (char*)"COND: Create cond %s.\n", nomDebug);
		Condition *cond = new Condition(nomDebug);
		
		//Ajout du verrou dans la liste des objets crées par le système
		objNumber = g_object_ids->AddObject(cond);
		g_machine->WriteIntRegister(2, objNumber);
		g_syscall_error->SetMsg((char*)"",NoError);
	...
	case SC_COND_DESTROY:{
		...
		int32_t numCond = g_machine->ReadIntRegister(4);
		Condition *cond = (Condition *)g_object_ids->SearchObject(numCond);
		
		if(cond && (cond->typeId == CONDITION_TYPE_ID)){
		
			DEBUG('e', (char*)"COND: DESTROY on condition %s.\n", cond->getName());
			
			//Suppression de l'objet ainsi que ses références
			g_object_ids->RemoveObject(numCond);
			delete cond;
			...
			DEBUG('e', (char*)"COND: Error on DESTROY.\n");
			g_machine->WriteIntRegister(2,-1);
	...
	case SC_COND_WAIT:{
		...
		if(cond && (cond->typeId == CONDITION_TYPE_ID)){
			...
			DEBUG('e', (char*)"COND: WAIT on condition %s.\n", cond->getName());
			...
			DEBUG('e', (char*)"COND: Error on WAIT.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidConditionId);
		}
	...
	case SC_COND_SIGNAL:{
		...
		if(cond && (cond->typeId == CONDITION_TYPE_ID)){
			...
			DEBUG('e', (char*)"COND: SIGNAL on condition %s.\n", cond->getName());
			...
			DEBUG('e', (char*)"COND: Error on SIGNAL.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidConditionId);
		}
	...
	case SC_COND_BROADCAST:{
		...
		if(cond && (cond->typeId == CONDITION_TYPE_ID)){
			...
			DEBUG('e', (char*)"COND: BROADCAST on condition %s.\n", cond->getName());
			...
			DEBUG('e', (char*)"COND: Error on BROADCAST.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidConditionId);
		}
	...
	case SC_BAR_CREATE:{
		...
		//Récupération des paramètres
		char nomDebug[256];
		int objNumber, limiteProc;
		
		GetStringParam(g_machine->ReadIntRegister(4), nomDebug, 256);
		limiteProc = g_machine->ReadIntRegister(5);

		if(limiteProc > 0){
		
			DEBUG('e', (char*)"BAR: Create barrier %s.\n", nomDebug);
			Barriere *bar = new Barriere(nomDebug, limiteProc);
		
			//Ajout du verrou dans la liste des objets crées par le système
			objNumber = g_object_ids->AddObject(bar);
			g_machine->WriteIntRegister(2, objNumber);
			g_syscall_error->SetMsg((char*)"",NoError);
		}
		else{
			...
			g_machine->WriteIntRegister(2, -1);
			g_syscall_error->SetMsg((char*)"La limite de processus spécifiée doit être positive", InvalidParameter);
		}
	...
	case SC_BAR_DESTROY:{
		...
		int32_t numBar = g_machine->ReadIntRegister(4);
		Barriere *bar = (Barriere *)g_object_ids->SearchObject(numBar);
		
		if(bar && (bar->typeId == BARRIERE_TYPE_ID)){
		
			DEBUG('e', (char*)"BAR: Destroy barrier %s.\n", bar->getName());
			
			//Suppression de l'objet ainsi que ses références
			g_object_ids->RemoveObject(numBar);
			delete bar;
			...
			DEBUG('e', (char*)"BAR: Error on DESTROY.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidBarrierId);
		}
	...
	case SC_BAR_WAIT:{
		...
		if(bar && (bar->typeId == BARRIERE_TYPE_ID)){
			...
			DEBUG('e', (char*)"BAR: Wait on barrier %s.\n", bar->getName());
			...
			DEBUG('e', (char*)"BAR: Error on WAIT.\n");
			g_machine->WriteIntRegister(2,-1);
			g_syscall_error->SetMsg((char*)"",InvalidBarrierId);
		}
	...
	case SC_MMAP:{
		...
		OpenFile *f = (OpenFile *)g_object_ids->SearchObject(g_machine->ReadIntRegister(4));
		int tailleMap = g_machine->ReadIntRegister(5);
		int retourMmap;
		
		if(f && tailleMap > 0){
		
			retourMmap = g_current_thread->GetProcessOwner()->addrspace->Mmap(f, tailleMap);

			switch(retourMmap){
			
				case -1:
					
					g_syscall_error->SetMsg((char*)"Erreur d'allocation mémoire", OutOfMemory);
					g_machine->WriteIntRegister(2, -1);
					break;
					
				case -2:
				
					g_syscall_error->SetMsg((char*)"Erreur (nombre de fichier mappé au maximum)", OutOfMemory);
					g_machine->WriteIntRegister(2, -1);
					break;
					
				default:
				
					g_machine->WriteIntRegister(2, retourMmap);
					g_syscall_error->SetMsg((char*)"", NoError);
					break;
			}
		}
