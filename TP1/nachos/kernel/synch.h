...
Définit l'outil de synchronisation Barriere qui laisse passer les processus qu'elle stocke une fois que le nombre ne attente est assez important

Wait() fait attendre un processus à la barrière ou débloque l'ensemble des processus si le nombre requis est atteint
*/
class Barriere{

	public:
	
		Barriere(char *debugName, int limiteBarriere);
		~Barriere();
	...
	private:
		...
		char *name;
		Semaphore *mutex, *attente, *procsRelaches;
		int limiteBarriere, nbProcs;
