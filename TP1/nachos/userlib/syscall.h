...
#define SC_BAR_CREATE	 36 
#define SC_BAR_DESTROY	 37 
#define SC_BAR_WAIT	 38 
...
/* System calls concerning barrier management */
typedef int BarriereId;

/* Create a new barrier variable 

	limiteProc define the limit of processes/threads to open the barrier (must be positive)
	return -1 if an error occured
*/
BarriereId BarriereCreate(char * debug_name, int limiteProc);

/* Destroy a barrier variable.
   Return a negative number if an error ocurred.
*/
int BarriereDestroy(BarriereId id);

/* Do the operation Wait on a barrier variable.
   Returns a negative number if an error ocurred.
*/
int BarriereWait(BarriereId id);
